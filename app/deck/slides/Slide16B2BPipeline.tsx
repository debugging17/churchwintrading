...
export function Slide16B2BPipeline({ isActive }: { isActive: boolean }) {
  ...
  return (
    <div className="w-full h-full flex flex-col py-6 md:py-10 px-8 md:px-20 bg-brand-cream pb-[calc(var(--footer-height)+4rem)] pt-[calc(2rem+var(--header-height,0px))] md:pt-[calc(3rem+var(--header-height,0px))] overflow-y-auto relative">
      ...
      <motion.div initial="hidden" animate={animState} variants={sectionTitleStagger} className="w-full max-w-[90rem] mx-auto my-auto flex flex-col gap-6 z-10">
        
        {/* Top: Pitch */}
        <div className="w-full flex flex-col md:flex-row items-end justify-between gap-6 mb-2">
          <div className="max-w-xl">
            <motion.h2 variants={slideFromLeft} className="font-display text-4xl md:text-5xl lg:text-5xl font-extrabold text-brand-navy leading-none mb-3 uppercase tracking-tighter">
              The B2B <br />
              <span className="text-brand-orange italic font-light">Demand Engine.</span>
            </motion.h2>
            
            <motion.p variants={fadeUp} className="text-base md:text-lg text-brand-navy/70 font-primary border-l-2 border-brand-navy/20 pl-6 italic mb-0 leading-relaxed">
          ...
          <motion.div variants={fadeUp} className="flex gap-3">
            {[
              ...
              <div key={idx} className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-full border border-brand-navy/10 shadow-sm">
                 ...
                 <span className="text-[9px] font-black uppercase tracking-widest text-brand-navy">{item.title}</span>
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
           ...
           <div className="lg:col-span-4 h-full">
              <motion.div variants={fadeUp} className="h-full p-6 bg-brand-navy text-white rounded-[2.5rem] shadow-2xl relative overflow-hidden border-b-8 border-brand-orange flex flex-col justify-center min-h-[420px]">
                 <div className="absolute top-0 right-0 p-6 opacity-10">
                    <Target className="w-16 h-16 text-brand-orange" />
                 </div>
                 <p className="text-[9px] uppercase tracking-[0.5em] text-brand-orange font-black mb-3">Strategic Outcome</p>
                 <h3 className="text-lg md:text-xl font-display font-bold mb-4 tracking-tight leading-tight">100% Digital Traceability from Outreach to Agreement.</h3>
                 <div className="space-y-4">
                    <div className="flex items-center gap-6">
                       <div>
                          <p className="text-white/40 text-[7px] uppercase font-black mb-1 tracking-widest">Qualified Leads</p>
                          <p className="text-2xl md:text-2xl font-display font-black text-brand-orange">12/mo</p>
                       </div>
                       <div className="w-[1px] h-6 bg-white/10" />
                       <div>
                          <p className="text-white/40 text-[7px] uppercase font-black mb-1 tracking-widest">Conversion</p>
                          <p className="text-2xl md:text-2xl font-display font-black text-brand-orange">22%</p>
                       </div>
                    </div>
                    <div className="pt-4 border-t border-white/10">
                       <p className="text-white/40 text-[7px] uppercase font-black mb-1 tracking-widest">Target CAC</p>
                       <p className="text-2xl md:text-2xl font-display font-black text-brand-orange">$420</p>
                    </div>
                 </div>
              </motion.div>
           </div>
