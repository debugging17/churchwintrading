...
export function Slide14WhoOwnsSpace() {
  ...
  const renderIcon = (val: boolean | string) => {
    ...
    if (val === false) return <XCircle className="w-5 h-5 text-brand-white/20 mx-auto" />;
    ...
    return <span className="text-brand-white/80">{val}</span>;
  };

  return (
    <div className="w-full h-full flex flex-col justify-center px-10 md:px-16 bg-brand-navy">
      <motion.div initial="hidden" animate={animState} variants={sectionTitleStagger} className="w-full max-w-7xl mx-auto">
        <motion.div variants={fadeUp} className="mb-12">
          <h2 className="font-display text-4xl md:text-5xl font-bold text-brand-white mb-4">
            Four players own this market. <span className="text-brand-orange font-light">None are Ghanaian at scale.</span>
          </h2>
          <p className="font-primary text-brand-white/60 max-w-4xl text-lg">
        ...
        {/* Comparison Table */}
        <motion.div variants={fadeUp} className="w-full overflow-x-auto bg-brand-white/5 border border-brand-white/10 rounded-xl backdrop-blur-sm shadow-2xl mb-8">
          <table className="w-full text-left font-primary text-sm whitespace-nowrap">
            <thead>
              <tr className="border-b border-brand-white/10 uppercase font-display text-xs tracking-widest text-[#ff8022]">
                ...
                <th className="px-6 py-4 font-semibold text-center bg-[#ff8022]/10 text-brand-orange">Private Label</th>
            ...
            <tbody>
              {competitors.map((comp, idx) => {
                ...
                return (
                  <tr key={idx} className={`border-b border-brand-white/5 transition-colors ${isChurchwin ? "bg-brand-orange/10 border-brand-orange/30 shadow-[inset_4px_0_0_#ff6a00]" : "hover:bg-brand-white/5"}`}>
                    <td className={`px-6 py-5 font-semibold ${isChurchwin ? "text-brand-orange text-base" : "text-brand-white"}`}>{comp.name}</td>
                    <td className="px-6 py-5 text-center text-lg">{comp.hq}</td>
                    <td className="px-6 py-5 text-brand-white/80">{comp.model}</td>
                    ...
                    <td className={`px-6 py-5 text-center ${isChurchwin ? "bg-brand-orange/10 border-x border-brand-orange/20" : ""}`}>
                    ...
                    <td className={`px-6 py-5 ${isChurchwin ? "text-brand-orange/80 italic" : "text-brand-white/50"}`}>{comp.lacks}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        {/* The Paradox Callout */}
        <motion.div variants={fadeUp} className="border-l-4 border-brand-orange pl-6 py-2 bg-gradient-to-r from-brand-orange/5 to-transparent rounded-r-md">
          <p className="font-primary font-medium text-brand-white/90 md:text-lg">
