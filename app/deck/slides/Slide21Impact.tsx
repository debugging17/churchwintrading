...
export function Slide21Impact({ isActive }: { isActive: boolean }) {
  ...
  return (
    <div className="w-full h-full flex flex-col py-6 md:py-10 px-8 md:px-20 bg-brand-navy pb-[calc(var(--footer-height)+2rem)] pt-[calc(2rem+var(--header-height,0px))] md:pt-[calc(3rem+var(--header-height,0px))] overflow-y-auto relative text-brand-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <motion.div 
              key={i}
              variants={fadeUp}
              className="p-10 bg-white/5 backdrop-blur-xl border border-white/10 rounded-[40px] shadow-2xl group hover:border-brand-orange transition-all text-left"
            >
